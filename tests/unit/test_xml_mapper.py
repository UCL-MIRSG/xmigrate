...
def test_rewrite_uris_replaces_source_path() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("file", {"URI": "/archive/src/src_proj/file.dcm"})
...
def test_rewrite_uris_does_nothing_without_uri() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("file", {"other": "value"})
    mapper.rewrite_uris(element, "/archive/src/src_proj", "/archive/dst/dst_proj")
...
def test_rewrite_uris_raises_if_source_path_missing() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("file", {"URI": "/archive/other/file.dcm"})
...
def test_rewrite_uris_only_replaces_first_occurrence() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("file", {"URI": "/archive/src/src_proj/archive/src/src_proj/file.dcm"})
    mapper.rewrite_uris(element, "/archive/src/src_proj", "/archive/dst/dst_proj")
...
def test_map_xml_project_updates_id_and_secondary_id() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("Project", {"ID": "src_proj", "secondary_ID": "src_secondary"})
...
def test_map_xml_project_updates_name() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("Project", {"ID": "src_proj", "secondary_ID": "src_secondary"})
    name = ElementTree.SubElement(element, f"{{{xmigrate.XnatNS.xnat}}}name")
...
def test_map_xml_deletes_unwanted_tags() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("Subject", {"project": "src_proj"})
    ElementTree.SubElement(element, f"{{{xmigrate.XnatNS.xnat}}}experiments")
    ElementTree.SubElement(element, f"{{{xmigrate.XnatNS.xnat}}}sharing")
...
def test_map_xml_updates_project_attribute() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("Subject", {"project": "src_proj", "ID": "sub_1"})
    mapper.map_xml(element, xmigrate.XnatType.subject)
...
def test_map_xml_deletes_id_for_non_project_non_scan() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("Subject", {"ID": "sub_1", "project": "src_proj"})
    mapper.map_xml(element, xmigrate.XnatType.subject)
...
def test_map_xml_preserves_id_for_scan() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("Scan", {"ID": "scan_1", "project": "src_proj"})
...
def test_map_xml_remaps_subject_id_tag() -> None:
    ...
    mapper = make_mapper()
    mapper.id_map[xmigrate.XnatType.subject]["src_sub_1"] = "dst_sub_1"
    element = ElementTree.Element("Experiment", {"project": "src_proj", "ID": "exp_1"})
    subject_id = ElementTree.SubElement(element, f"{{{xmigrate.XnatNS.xnat}}}subject_ID")
...
def test_map_xml_rewrites_file_uris() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element("Resource", {"project": "src_proj"})
    file_elem = ElementTree.SubElement(
        element, f"{{{xmigrate.XnatNS.xnat}}}file", {"URI": "/archive/src/src_proj/file.dcm"}
    )
...
def test_map_xml_fixes_mr_scan_modality() -> None:
    ...
    mapper = make_mapper()
    element = ElementTree.Element(f"{{{xmigrate.XnatNS.xnat}}}imageScanData", {"ID": "scan_1", "project": "src_proj"})
    modality = ElementTree.SubElement(element, f"{{{xmigrate.XnatNS.xnat}}}modality")
    modality.text = "MR"
...
def test_map_xml_uses_other_scan_for_multiple_modalities() -> None:
    ...
    element = ElementTree.Element(f"{{{xmigrate.XnatNS.xnat}}}imageScanData", {"ID": "scan_1", "project": "src_proj"})
    for mod in ("MR", "CT"):
        m = ElementTree.SubElement(element, f"{{{xmigrate.XnatNS.xnat}}}modality")
        m.text = mod
    result = mapper.map_xml(element, xmigrate.XnatType.scan)
    assert result.tag == "xnat:OtherDicomScan"
